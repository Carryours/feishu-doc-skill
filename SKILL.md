...
3. 读取正文时：
   - 默认直接运行 `scripts/read_feishu_url_md.py <飞书链接>`，不要先手动提取 token，脚本会自己处理链接和 token。
   - 如果用户只想调试结构化返回，运行 `scripts/read_feishu_url.py <飞书链接>`。
   - 只有在需要看原始接口结构时，再运行 `scripts/read_feishu_doc.py <doc_or_wiki_token>`。
