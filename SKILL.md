...
2. 优先走 API。
   ...
   - 如果没有用户 token，但有 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`，先运行 `scripts/feishu_oauth_server.js` 获取开发者自己的 `user_access_token`。

3. 读取正文时：
   - 运行 `scripts/extract_feishu_doc_id.py` 提取 token。
   - 运行 `scripts/read_feishu_doc.py` 获取 wiki 节点信息、文档元信息和正文块。
   - 如果用户要继续做需求分析、写方案、拆任务，优先运行 `scripts/read_feishu_url_md.py`。
   - 如果用户只想调试结构化返回，运行 `scripts/read_feishu_url.py`。

4. 插入图片时：
   - 只支持“本地图片路径”，不直接复用聊天里的图片附件。
   - 运行 `scripts/insert_feishu_local_image.py <飞书文档链接或 token> <本地图片绝对路径>`。
...
- 提取 token：`scripts/extract_feishu_doc_id.py`
- 本地 OAuth 回调：`scripts/feishu_oauth_server.js`
- 读取正文：`scripts/read_feishu_doc.py`
- 一键读取与摘要：`scripts/read_feishu_url.py`
- 一键读取为 Markdown：`scripts/read_feishu_url_md.py`
- 插入本地图片：`scripts/insert_feishu_local_image.py`
